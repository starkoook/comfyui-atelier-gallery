...
function renderBrowse(node, data) {
  ...
  if (count) count.textContent = `${folders.length} folders / ${images.length} images`;
...
function buildWidget(node) {
  ...
  root.innerHTML = `
    <div class="atg-bar">
      ...
      <input class="atg-path" type="text" placeholder="Folder path  D:\\pics" />
      <button type="button" class="primary" data-act="go">Open</button>
  ...
  try {
    ...
    if (dom) dom.computeSize = (width) => [width || 480, 460];
  } catch (err) {
  ...
  if (!node.size || node.size[0] < 420) {
    node.setSize([Math.max(node.size?.[0] || 0, 520), Math.max(node.size?.[1] || 0, 640)]);
  }
  goPath(node, start);
}
